...
def parse_args():
    ...
    parser.add_argument('--num_data', type=int, default= 1000, help="number of data")
    parser.add_argument('--metric', type=str, default='robust', help="robust or dp/eop")
    ...
    parser.add_argument('--weight_scheme', type=str, default='hard', help="'soft-2.3', 'soft-2.3','soft-2.2','soft-2.1', 'soft-2.0'")
...
def pre_main(args):
    ...
    # Ensure other data also uses standardization
    ...
    # Calculate robustness accuracy and loss values
    if args.model_type != 'nn':
        val_rob_acc, val_rob_loss = calc_robust_acc(model, data.x_val, data.y_val, 'val', 'pre')
        test_rob_acc, _ = calc_robust_acc(model, data.x_test, data.y_test, 'test', 'pre')
    ...
    np.save('results/SEC-5-1-2-2/robust/util_infl.npy', util_pred_infl)
    np.save('results/SEC-5-1-2-2/robust/robust_infl.npy', robust_pred_infl)
    ...
    util_infl_df.to_csv('results/SEC-5-1-2-2/robust/util_infl.csv', index=False)
    robust_infl_df.to_csv('results/SEC-5-1-2-2/robust/robust_infl.csv', index=False)

    # Save combined influence values
    combined_df = pd.DataFrame({
        'util_influence': util_pred_infl,
        'robust_influence': robust_pred_infl
    })
    combined_df.to_csv('results/SEC-5-1-2-2/robust/influence_values.csv', index=False)
    ...
    return data, x_train_pca, model, ori_util_loss_val, val_rob_acc, val_rob_loss
...
def post_main(args, model, data, weights):
    ...
    # Calculate robustness accuracy
    if args.model_type != 'nn':
        val_rob_acc, val_rob_loss = calc_robust_acc(post_model, data.x_val, data.y_val, 'val', 'post')
        test_rob_acc, _ = calc_robust_acc(post_model, data.x_test, data.y_test, 'test', 'post')
        print("Validation set robustness accuracy -> ", val_rob_acc)
        print("Test set robustness accuracy -> ", test_rob_acc)
    else:
        val_rob_acc = calc_robust_acc_nn(post_model, data.x_val, data.y_val, 'val', 'post')
        test_rob_acc = calc_robust_acc_nn(post_model, data.x_test, data.y_test, 'test', 'post')
        print("Validation set robustness accuracy -> ", val_rob_acc)
        print("Test set robustness accuracy -> ", test_rob_acc)

    post_util_loss_val = post_model.log_loss(data.x_val, data.y_val)

    return data, post_model, post_util_loss_val, val_rob_acc, val_rob_loss
...
def plot_results(data, x_train_pca, pre_model, post_model, x_train_post_pca, weights, args):
    ...
    if args.weight_scheme == "hard":
        ...
        for i in range(len(x_train_pca)):
            if deleted_mask[i]:
                ax.scatter(x_train_pca[i, 0], x_train_pca[i, 1],
                          ...
                          marker='x', s=400, linewidths=3, alpha=0.6)
    else:
        ...
        alphas = np.where(weights >= 0,
                         0.7 + 0.3 * weights_norm,
                         0.1 + 0.3 * weights_norm)
        for i in range(len(x_train_pca)):
            ax.scatter(x_train_pca[i, 0], x_train_pca[i, 1],
                      c=[BLUE_COLOR if data.y_train[i] == 0 else RED_COLOR],
    ...
    # Display accuracy, robustness and loss comparison before and after
    metrics_text = (f'Pre-Unlearning:\n'
                   f'  Test Accuracy: {pre_accuracy*100:.2f}%\n'
                   f'  Robustness Accuracy: {pre_rob_acc*100:.2f}%\n'
                   f'Post-Unlearning:\n'
                   f'  Test Accuracy: {post_accuracy*100:.2f}%\n'
                   f'  Robustness Accuracy: {post_rob_acc*100:.2f}%')
    ...
    plt.tight_layout()
    plt.savefig('results/SEC-5-1-2-2/robust/decision_boundaries_{}_{}.pdf'.format(args.weight_scheme,args.metric), 
...
if __name__ == "__main__":
    ...
    fix_seed(args.seed)
    os.makedirs('results/SEC-5-1-2-2/robust', exist_ok=True)
    
    # 设置metric后缀
    metric_suffix = '_robust' if args.metric == 'robust' else '_fair'

    # Run pre-training
    data, x_train_pca, pre_model, ori_util_loss_val, val_rob_acc, val_rob_loss = pre_main(args)
    
    # Load influence values
    robust_infl = np.load('results/SEC-5-1-2-2/robust/robust_infl.npy')
    util_infl = np.load('results/SEC-5-1-2-2/robust/util_infl.npy')
    weights = np.zeros(len(robust_infl))
    

    k = 0.1 if args.dataset == "adult" and args.num_data == 1000 else 0.1
    num_points_to_delete = int(k * len(robust_infl))
    points_to_delete = np.argsort(robust_infl)[:num_points_to_delete]
    weights[points_to_delete] = -1
    data_post, post_model, hard_post_util_loss_val, hard_post_rob_acc, hard_post_rob_loss = post_main(args, pre_model, data, weights)
    # args.weight_scheme = "hard" if args.dataset != "adult" and args.weight_scheme != "hard" else "soft-2.3"

    if args.weight_scheme == "hard":
        miu = 0.001
        data_post, post_model, post_util_loss_val, post_rob_acc, post_rob_loss = post_main(args, pre_model, data, weights)
        
        np.save(f'results/SEC-5-1-2-2/hard_weights{metric_suffix}.npy', weights)
        weights_df = pd.DataFrame(weights)
        weights_df.to_csv(f'results/SEC-5-1-2-2/hard_weights{metric_suffix}.csv', index=False, header=False)

    elif args.weight_scheme == "soft-2.0":
        """ Soft weights2.0 with fixed lambda"""
        iter_num = 50
        ...
        #     weights = (robust_infl ) / (2 * lambda_reg)
        #     # print('Weighted Robustness',-np.dot(weights.T, robust_infl))
        ...
        #     # print(f"Robustness Accuracy: {post_rob_acc:.4f}")
        #     # print(f"Best Robustness Accuracy: {best_rob_acc:.4f}")

        #     if post_rob_acc >= best_rob_acc:  
        #         best_rob_acc = post_rob_acc
        #         best_lambda = lambda_reg
        #         best_weights = weights
        #         best_results = (data_post, post_model, post_util_loss_val, post_rob_acc, post_rob_loss)
        # print(f"\nBest lambda for Scheme 2.2: {best_lambda:.8f}")
        # print(f"Best Utility Loss: {best_results[2]:.8f}")
        # print(f"Best Robustness Accuracy: {best_results[3]:.4f}")
        # args.weights_regularizer = best_lambda
        # weights = best_weights
        # data_post, post_model, post_util_loss_val, post_rob_acc, post_rob_loss = best_results
        lambda_reg =  0.1
        best_lambda = lambda_reg
        weights = (robust_infl ) / (2 * lambda_reg)
        data_post, post_model, post_util_loss_val, post_rob_acc, post_rob_loss = post_main(args, pre_model, data, weights)
        ...
        np.save(f'results/SEC-5-1-2-2/Analytical_weights_2{metric_suffix}_{lambda_reg:.8f}.npy', weights)
        weights_df = pd.DataFrame(weights)
        weights_df.to_csv(f'results/SEC-5-1-2-2/Analytical_weights_2{metric_suffix}_{lambda_reg:.8f}.csv', index=False, header=False)

    elif args.weight_scheme == "soft-2.1":
        """ Soft weights2.1 with fixed lambda """
        lambda_reg = 0.1
        beta2 = 1- 2* lambda_reg * np.linalg.norm(val_rob_loss) / (np.linalg.norm(robust_infl)**2)
        weights = (robust_infl * np.linalg.norm(val_rob_loss)) /  (np.linalg.norm(robust_infl)**2)
        print('Weighted Robustness',-np.dot(weights.T, robust_infl))
        print('Weighted Utility',-np.dot(weights.T, util_infl))            
        ...
        # 保存结果
        np.save(f'results/SEC-5-1-2-2/Analytical_weights_1{metric_suffix}_{lambda_reg:.8f}.npy', weights)
        weights_df = pd.DataFrame(weights)
        weights_df.to_csv(f'results/SEC-5-1-2-2/Analytical_weights_1{metric_suffix}_{lambda_reg:.8f}.csv', index=False, header=False)
        ...
        for lambda_reg in lambda_candidates:
            beta1 = - np.dot(robust_infl.T, util_infl) / (np.linalg.norm(util_infl)**2)
            if beta1 < 0:
                print(f"Skipping λ = {lambda_reg:.8f} because beta2 = {beta1:.8f} < 0")
                continue
            weights = (robust_infl + beta1 * util_infl) / (2 * lambda_reg)
            # print('Weighted Robustness',-np.dot(weights.T, robust_infl))
            # print('Weighted Utility',-np.dot(weights.T, util_infl))
            data_post, post_model, post_util_loss_val, post_rob_acc, post_rob_loss = post_main(args, pre_model, data, weights)
            # print(f"\nλ = {lambda_reg:.8f}:")
        ...
        weights = best_weights
        data_post, post_model, post_util_loss_val, post_rob_acc, post_rob_loss = best_results
        np.save(f'results/SEC-5-1-2-2/Analytical_weights_2{metric_suffix}_{best_lambda:.8f}.npy', weights)
        weights_df = pd.DataFrame(weights)
        weights_df.to_csv(f'results/SEC-5-1-2-2/Analytical_weights_2{metric_suffix}_{best_lambda:.8f}.csv', index=False, header=False)
        ...
        denominator = (np.linalg.norm(robust_infl)**2 * np.linalg.norm(util_infl)**2 - 
                      (np.dot(robust_infl, util_infl))**2) 
        # Calculate beta1
        beta1 = -(2 * lambda_reg * np.linalg.norm(val_rob_loss) * np.linalg.norm(util_infl)**2 * np.dot(robust_infl.T, util_infl)) / (denominator * np.linalg.norm(util_infl)**2)
        # Calculate beta2
        beta2 = 1 - (2 * lambda_reg * np.linalg.norm(val_rob_loss) * np.linalg.norm(util_infl)**2) / denominator 
        # Calculate weights epsilon
        weights = ((1 - beta2) * robust_infl + beta1 * util_infl) / (2 * lambda_reg)
        print('Weighted Robustness',-np.dot(weights.T, robust_infl))
        print('Weighted Utility',-np.dot(weights.T, util_infl))
        ...
        # 保存结果
        np.save(f'results/SEC-5-1-2-2/Analytical_weights_3{metric_suffix}_{lambda_reg:.8f}.npy', weights)
        weights_df = pd.DataFrame(weights)
        weights_df.to_csv(f'results/SEC-5-1-2-2/Analytical_weights_3{metric_suffix}_{lambda_reg:.8f}.csv', index=False, header=False)
    ...
    # Calculate pre-model accuracy and robustness accuracy
    ...
    if args.model_type != 'nn':
        pre_rob_acc, _ = calc_robust_acc(pre_model, data.x_val, data.y_val, 'test', 'pre')
    else:
        pre_rob_acc = calc_robust_acc_nn(pre_model, data.x_val, data.y_val, 'test', 'pre')

    # Calculate post-model accuracy and robustness accuracy
    ...
    if args.model_type != 'nn':
        post_rob_acc, _ = calc_robust_acc(post_model, data.x_val, data.y_val, 'test', 'post')
    else:
        post_rob_acc = calc_robust_acc_nn(post_model, data.x_val, data.y_val, 'test', 'post')
    ...
    print(f"Pre-model robustness accuracy: {pre_rob_acc:.4f}")
    print(f"Post-model accuracy: {post_accuracy:.4f}")
    print(f"Post-model robustness accuracy: {post_rob_acc:.4f}")

    # Call plot_results to draw
    plot_results(data, x_train_pca, pre_model, post_model, x_train_pca, 
                robust_infl if weights is None else weights, args)