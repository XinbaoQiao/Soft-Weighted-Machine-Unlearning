...
def post_main(args):
    ...
    # train_data_df.to_csv('results/SEC-5-1-0-1/train_data_{}.csv'.format(args.points_to_delete), index=True)
...
if __name__ == "__main__":
    ...
    os.makedirs('results/SEC-5-1-0-1', exist_ok=True)
    ...
    # Save actual values
    np.save('results/SEC-5-1-0-1/util_actual.npy', np.array(util_diff))
    np.save('results/SEC-5-1-0-1/fair_{}_actual.npy'.format(args.metric), np.array(fair_diff))
    np.save('results/SEC-5-1-0-1/robust_actual.npy', np.array(robust_diff))
    ...
    np.save('results/SEC-5-1-0-1/util_infl.npy', util_pred_infl)
    np.save('results/SEC-5-1-0-1/fair_{}_infl.npy'.format(args.metric), fair_pred_infl)
    np.save('results/SEC-5-1-0-1/robust_infl.npy', robust_pred_infl)
    ...
    util_df.to_csv('results/SEC-5-1-0-1/util_actual.csv', index=False)
    fair_df.to_csv('results/SEC-5-1-0-1/fair_{}_actual.csv'.format(args.metric), index=False)
    robust_df.to_csv('results/SEC-5-1-0-1/robust_actual.csv', index=False)
    ...
    try:
        ...
        util_infl_df.to_csv('results/SEC-5-1-0-1/util_infl.csv', index=False)
        fair_infl_df.to_csv('results/SEC-5-1-0-1/fair_{}_infl.csv'.format(args.metric), index=False)
        robust_infl_df.to_csv('results/SEC-5-1-0-1/robust_infl.csv', index=False)
