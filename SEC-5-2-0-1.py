...
def parse_args():
    ...
    parser.add_argument('--metric', type=str, default="dp", help="dp/robust")
...
def pre_main(args):
    ...
    np.save('results/SEC-5-2-0-1/linear_train_indices.npy', linear_train_indices)
...
def create_weighted_dataloaders(data, weights, x_train_linear, y_train_linear, args):
    ...
    x_train = torch.FloatTensor(x_train_linear)  # Use linear layer training data
    y_train = torch.LongTensor(y_train_linear)   # Use linear layer training data
    
    # Get corresponding sensitive attributes
    linear_train_indices = np.load('results/SEC-5-2-0-1/linear_train_indices.npy')
    ...
    save_path = f'results/SEC-5-2-0-1/sample_info_{weight_type}.csv'
    ...
    if args.weight_scheme == "hard":
        ...
        forget_mask = weights == -1  # Weight -1 for data to forget
        retain_mask = weights == 0  # Weight 0 for data to retain
...
def calculate_weights_scheme_2_0(fair_infl, util_infl, ori_fair_loss_val, ori_util_loss_val):
    ...
    # If no suitable weights found, use uniform weights
...
def calculate_weights_scheme_2_2(fair_infl, util_infl, ori_fair_loss_val, ori_util_loss_val):
    ...
    for lambda_reg in lambda_candidates:
        ...
        # Remove beta1 < 0 check, because negative values are also valid
...
if __name__ == "__main__":
    ...
    args.unlearn_method = "cuk" 
    #######################
    # Create results directory if it doesn't exist
    os.makedirs('results/SEC-5-2-0-1', exist_ok=True)
    ...
    # Select appropriate influence based on task type
    ...
    if not hasattr(model, 'model'):
        # Use a random input to initialize model
    ...
    if args.metric == "dp":
        # Keep original positive and negative values for display, but use absolute value relative change to calculate improvement percentage
        ...
        formatted_results['Pre-unlearning'].append(f"{pre_dp:.6f}")  # Keep original positive and negative values
        formatted_results['Post-unlearning'].append(f"{post_dp:.6f}")  # Keep original positive and negative values
    ...
    # Save hyperparameters (removable code block)
    hyperparams = vars(args)  # Convert args object to dictionary containing all parameters
    
    # Remove parameters not to save (e.g., device)
