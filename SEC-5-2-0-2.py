...
from utils import fix_seed, save2csv, AverageMeter, accuracy  # Add required utils functions
...
def parse_args():
    ...
    parser.add_argument('--metric', type=str, default="robust", help="dp/eop/robust")
    ...
    parser.add_argument('--weight_scheme', type=str, default="soft-2.3", help="hard/soft-2.1/soft-2.2/soft-2.3")
...
def pre_main(args):
    ...
    np.save('results/SEC-5-2-0-2/linear_train_indices.npy', linear_train_indices)
    ...
    if args.metric == "eop":
        ...
        # For robust metric, set ori_fair_loss_val to None
        ori_fair_loss_val = None
        fair_loss_total_grad = None  
    ...
    if args.metric == "robust":
        ...
        robust_pred_infl = None

    end_time = time.time()  # Record end time
    print(f"Total time taken for influence computation: {end_time - start_time:.6f} seconds")
...
def create_weighted_dataloaders(data, weights, x_train_linear, y_train_linear, args):
    ...
    x_train = torch.FloatTensor(x_train_linear)  
    y_train = torch.LongTensor(y_train_linear)   
    
    # Get corresponding sensitive attributes
    linear_train_indices = np.load('results/SEC-5-2-0-2/linear_train_indices.npy')
    ...
    save_path = f'results/SEC-5-2-0-2/sample_info_{weight_type}.csv'
    ...
    if args.weight_scheme == "hard":
        ...
        data_loaders = {
            ...
            "val": DataLoader(val_dataset, batch_size=args.batch_size, shuffle=False),
            "test": DataLoader(test_dataset, batch_size=args.batch_size, shuffle=False),
            "forget": DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True), 
            "retain": DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True)   
        }
...
if __name__ == "__main__":
    ...
    args.unlearn_method = "if" 
    #######################
    # Create results directory if it doesn't exist
    os.makedirs('results/SEC-5-2-0-2', exist_ok=True)
    ...
    print(f"Utility - Accuracy: {pre_accuracy:.2f}%")
    if args.metric == "dp":
        print(f"Fairness (DP) - Loss: {ori_fair_loss_val:.4f}")
    elif args.metric == "eop":
        print(f"Fairness (EOP) - Loss: {ori_fair_loss_val:.4f}")
    else:
        print(f"Robustness - Val Accuracy: {val_rob_acc*100:.4f}%")
        # Calculate and print test set robustness
        if args.model_type == 'logreg':
            ...
            test_rob_acc, _ = calc_robust_acc_nn(model, data.x_test, data.y_test, 'test', 'pre')
        print(f"Robustness - Test Accuracy: {test_rob_acc*100:.4f}%")
    ...
    if args.weight_scheme == "hard":
        ...
        weights, best_lambda = calculate_weights_scheme_2_3(
            ...
            ori_util_loss_val)
        end_time = time.time()
        print(f"Time taken for calculating weights using scheme 2.3: {end_time - start_time:.6f} seconds")
    ...
    if args.unlearn_method.lower() == "euk":
        ...
        print(f"Time taken for IF unlearning: {elapsed_time:.6f} seconds")
    else:
    ...
    if args.metric == "dp":
        print(f"Fairness (DP) - Val Loss: {final_metrics['val']['fairness']['dp']:.4f}")
    elif args.metric == "eop":
        print(f"Fairness (EOP) - Val Loss: {final_metrics['val']['fairness']['eop']:.4f}")
    elif 'robustness' in final_metrics['val']:
        print(f"Robustness - Val Accuracy: {final_metrics['val']['robustness']['accuracy']:.2f}%")
        # Calculate and print test set robustness using unlearn_model
        if args.model_type == 'logreg':
            from robust_fn import calc_robust_acc
            test_rob_acc, _ = calc_robust_acc(unlearn_model, data.x_test, data.y_test, 'test', 'post')
        else:  # 'nn' or 'resnet'
            from robust_fn_nn import calc_robust_acc_nn
            test_rob_acc, _ = calc_robust_acc_nn(unlearn_model, data.x_test, data.y_test, 'test', 'post')
        print(f"Robustness - Test Accuracy: {test_rob_acc*100:.4f}%")
    ...
    if args.metric == "dp":
        ...
        dp_improve = ((abs(pre_dp) - abs(post_dp)) / abs(pre_dp)) * 100
        ...
        fair_improve = ((ori_fair_loss_val - final_metrics['val']['fairness']['eop']) / ori_fair_loss_val) * 100
        ...
        rob_acc_improve = final_metrics['val']['robustness']['accuracy'] - val_rob_acc * 100
        print(f"Robustness Acc:      {val_rob_acc * 100:.4f}%         {final_metrics['val']['robustness']['accuracy']:.2f}%         {rob_acc_improve:>7.2f}%")
        
        formatted_results['Metric'].append('Robustness Accuracy')
        formatted_results['Pre-unlearning'].append(f"{val_rob_acc * 100:.4f}%")
    ...
    hyperparams = vars(args)  # Convert args object to dictionary, including all parameters
