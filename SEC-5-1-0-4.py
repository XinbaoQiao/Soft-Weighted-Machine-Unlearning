...
def pre_main(args):
    ...
    # Select robustness calculation function based on model type
...
if __name__ == "__main__":
    args = parse_args()
    os.makedirs('results/SEC-5-1-0-4', exist_ok=True)
    ...
    # Save CSV files
    util_df.to_csv('results/SEC-5-1-0-4/util_actual.csv', index=False)
    fair_df.to_csv('results/SEC-5-1-0-4/fair_{}_actual.csv'.format(args.metric), index=False)
    robust_df.to_csv('results/SEC-5-1-0-4/robust_actual.csv', index=False)

    # Save complete influence values
    np.save('results/SEC-5-1-0-4/util_infl_full.npy', util_pred_infl)
    np.save('results/SEC-5-1-0-4/fair_{}_infl_full.npy'.format(args.metric), fair_pred_infl)
    np.save('results/SEC-5-1-0-4/robust_infl_full.npy', robust_pred_infl)